...
distributions = {  # Create a dictionary with calculated theoretical distributions
    "Normal": stats.norm.pdf(
        x_values, *stats.norm.fit(column)
...
axes.hist(
    column, bins=70, alpha=0.5, label=column.name, density=True
)  # Histogram of the data, semi transparent thanks to alpha
axes.plot(
    x_values, distribution, label="Theoretical Distribution"
